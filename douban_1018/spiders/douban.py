...
class DoubanSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        formdata = {'source':'index_nav',
                    ...
                    'form_email':'jingdeshuaijiang@163.com',
                    'form_password':'wsZqaz123!@#',
                    'remember':'on',
                    'user_login':'登录'
                    }
        ...
        print('captcha_url:{}'.format( captcha_url))
        # print('captcha_url2:{}'.format( captcha_url2))

        if len(captcha_url):
            captcha = self.regonize_captcha(captcha_url[0])
            formdata['captcha-solution'] = captcha
            captcha_id = response.xpath("//input[@name='captcha-id']/@value").get()
            print('captcha_id:{}'.format(captcha_id))
    ...
    def parse_login_after(self, response):
        ...
        print('people_index_url:{}'.format( people_index_url))
